...
async function setupGame(accounts, ticTacToe) {
  ...
  logGasUsage("creating a game", tx)
  ...
  assertRejects(ticTacToe.join(1, {from: accounts[1], value: web3.toWei(1, 'ether')}), "Should not be able to join a game again")
...
async function makeMove(accounts, ticTacToe, currentPlayer, previousMove, moves, game, field, expectedPlayerReset = false) {
  logGasUsage("move number " + (parseInt(moves) + 1), await ticTacToe.makeMove(game, field, {from: accounts[currentPlayer - 1]}))
  ...
  assert.ok(previousMove <= lastMove, "last move should be after previous move: " + lastMove + " vs " + previousMove)
  moves++
...
contract('TicTacToe', function(accounts) {
    ...
    it('Schould create a game and cancel',  async () => {
        ...
        assertRejects(ticTacToe.join(1, {value: web3.toWei(1, 'ether')}), "Should not be able to join own game")
    ...
    it('Schould create, join and end a game with a winner',  async () => {
        ...
        assert.equal(await ticTacToe.senderPlayerIndex(1, {from: accounts[3]}), 0, "Unknown player should have index 0")
        ...
        // Start playing
        assertRejects(ticTacToe.makeMove(1, 9, {from: accounts[currentPlayer - 1]}), "Should not be able to make a move if to an non-existing field")
        ...
        assertRejects(ticTacToe.makeMove(1, 4, {from: accounts[(currentPlayer - 1)]}), "Should not be able to make a move on a occupied field")
    ...
    it('Schould create, join and end a game with a winner on last move',  async () => {
        await setupGame(accounts, ticTacToe)
        var currentPlayer = await ticTacToe.currentPlayerIndex(1)
        var lastMove = await ticTacToe.lastMove(1)
        var moves = await ticTacToe.currentMoves(1)
        ...
        var [currentPlayer, lastMove, moves] = await makeMove(accounts, ticTacToe, currentPlayer, lastMove, moves, 1, 7)
    ...
    it('Schould create, join and kick a player',  async () => {
        await setupGame(accounts, ticTacToe)
        var currentPlayer = await ticTacToe.currentPlayerIndex(1)
        var lastMove = await ticTacToe.lastMove(1)
        var moves = await ticTacToe.currentMoves(1)
        ...
        assertRejects(ticTacToe.cancel(1), "Should not be able to cancel a game after start")
        assertRejects(ticTacToe.punishCurrentPlayer(1, {from: accounts[(currentPlayer % 2)]}), "Should not be able to punish a player right after game start")
        ...
        await wait(3500, 1)
        assertRejects(ticTacToe.punishCurrentPlayer(1, {from: accounts[(currentPlayer % 2)]}), "Should not be able to punish a player right after game start")
        assert.ok(!(await ticTacToe.canCurrentPlayerBePunished(1, {from: accounts[(currentPlayer % 2)]})), "Player should not be punihsable before 1 hour")
        await wait(101, 1)
        assert.ok(await ticTacToe.canCurrentPlayerBePunished(1, {from: accounts[(currentPlayer % 2)]}), "Player should be punihsable after 1 hour")
        assertRejects(ticTacToe.punishCurrentPlayer(1, {from: accounts[(currentPlayer - 1)]}), "Current player should not be able to punish himself")
        await ticTacToe.punishCurrentPlayer(1, {from: accounts[(currentPlayer % 2)]})

        if (await ticTacToe.currentPlayerIndex(1) == 2) {
          assert.ok(account1Balance < web3.eth.getBalance(accounts[1]).toNumber(), "Kicking player should get all")
          assert.ok(account0Balance > web3.eth.getBalance(accounts[0]).toNumber(), "Kicked player should get nothing")
        } else {
          assert.ok(account0Balance < web3.eth.getBalance(accounts[0]).toNumber(), "Kicking player should get all")
          assert.ok(account1Balance > web3.eth.getBalance(accounts[1]).toNumber(), "Kicked player should get nothing")
        }
        assert.equal(await ticTacToe.currentGameState(1), 2)
        assert.equal(await web3.eth.getBalance(ticTacToe.address).toNumber(), 0)
    })
